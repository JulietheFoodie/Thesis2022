...
```{r}
state <- c("al", "fl", "ga", "il", "in", "ks", "ky", "md", "mn", "ms", "nv", "nm", "nc", "nd", "oh", "sc", "sd", "tn", "wv", "wi", "wy")

stateA <- c("al", "fl", "ga", "il", "in", "ks", "ky",  "mn", "ms",   "nc", "nd",  "sc", "sd", "tn", "wv", "wi", "wy")

stateB <- c("de", "md", "nv", "nm", "oh")
# "de"
```
...
```{r}
all_df12 <- rbind(all_df12A, all_df12B)
...
# List of Datasets
datalist14A[[i]] <- get(paste0(i, "14_df"))


}

# Combine Datasets
all_df14A <- do.call(rbind, datalist14A, )

# lowercase column names
names(all_df14A) <- tolower(names(all_df14A))

# Find Unique Column Names
unique_names <- unique(colnames(all_df14A))

# Keep Only Unique Column Names
all_df14A <- all_df14A[unique_names]
...
# List of Datasets
datalist14B[[i]] <- get(paste0(i, "14_df"))


}

# Combine Datasets
all_df14B <- do.call(rbind, datalist14B, )

# lowercase column names
names(all_df14B) <- tolower(names(all_df14B))

# Find Unique Column Names
unique_names <- unique(colnames(all_df14B))

# Keep Only Unique Column Names
all_df14B <- all_df14B[unique_names]


```

```{r}
all_df14 <- rbind(all_df14A, all_df14B)
...
all_df$treatment <- ifelse(state == "Alabama" | state == "Florida"| state == "Georgia" | state == "Kansas" | state == "Mississippi" | state == "North Carolina" | state == "South Dakota" | state == "South Carolina" | state == "Tennessee" | state == "Wisconsin"| state == "Wyoming", "0", "1" )
